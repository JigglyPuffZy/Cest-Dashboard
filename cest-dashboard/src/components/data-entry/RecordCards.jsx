...
function ComponentBadges({ components, darkMode, limit = 3 }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {components.slice(0, limit).map((c, i) => {
        ...
        return (
          <HoverTooltip key={`${key}-${i}`} content={COMPONENTS[key] || name} position="auto" darkMode={darkMode} delay={150}>
            <span
      ...
      {components.length > limit && (
        <span className="text-[10px] px-2 py-1 rounded-md font-medium" style={{ color: darkMode ? "#94a3b8" : "#64748b" }}>
          +{components.length - limit}
        </span>
      )}
    </div>
